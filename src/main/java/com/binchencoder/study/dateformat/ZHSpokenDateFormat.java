package com.binchencoder.study.dateformat;
...
public class ZHSpokenDateFormat {
    ...
    public static class AnalyzeResult {
        ...
        public AnalyzeResult(boolean matchDate, String beginDate, String endDate,
    ...
    public static AnalyzeResult analyzeStr(String str) {
        ...
        Pattern pattern = Pattern.compile(
            "(从?(?<beginField>[\\S\\s]*)(到|至)(?<endField>[\\S\\s]*))"
        );
    ...
    private static MatchDateResult matchMonth(String input, String year) {
        ...
        if (matcher.find()) {
            ...
            if (monthStd != null) {
                ...
                return new MatchDateResult(getFirstDayOfMonth(year, month),
                    getLastDayOfMonth(year, month), matcher.group());
            }
                ...
                if (monthOral.contains("年")) {
                    ...
                    if (monthOral.contains("半")) {
                        ...
                        if (!isPeriod(monthOral, "month")) {
                            if (monthOral.contains("上")) {
                                String[] arr = dateCalculationFull(tagBeginYear, null, null,
                                    "month", -1);
                                tagBeginYear = arr[0];
                                tagEndYear = arr[0];
                                begin = arr[1];
                            ...
                            String[] arr = dateCalculationFull(tagBeginYear, begin, null, "month",
                                amount);
                            tagBeginYear = arr[0];
                            begin = arr[1];
                        }
    ...
    private static MatchDateResult matchDay(String input, String year, String month) {
        Pattern pattern = Pattern.compile(
            "((?<dayStd>0?[1-9]|[0-2]\\d|3[0-1]|[一二三四五六七八九]|十[一二三四五六七八九]?|二十[一二三四五六七八九]?|三十[一]?)[-|\\/|日|号｜\\.|\\s])|"
                + "(?<dayOral>(([上中下]旬)|(现在|今)|(([昨今前明后]|(前|近|最近)[一二两三四五六七八九123456789]个?)[天日])|([上下这]?个?(星期|礼拜|周)[一二三四五六七天日1234567])))"
        );
            ...
            if (dayOral != null) {
                ...
                if (!isPeriod(dayOral, "day")) {
                    if (dayOral.contains("昨")) {
                        String[] arr = dateCalculationFull(tagBeginYear, tagBeginMonth, begin,
                            "day", -1);
                        tagBeginYear = arr[0];
                        tagEndYear = arr[0];
                        tagBeginMonth = arr[1];
     ...
     * @return
     */
    private static String[] getDayOfWeek(String dYear, String dMonth, String dDay, int amount,
        String dayOfWeek) {
        String yyyy = dYear == null ? getYearNow() : dYear;
        String MM = dMonth == null ? getMonthNow() : dMonth;
        String dd = dDay == null ? getDayNow() : dDay;
     ...
     * @return
     */
    private static String dateCalculation(String dYear, String dMonth, String dDay, String type,
        int amount) {
        Calendar calendar = Calendar.getInstance();
        String yyyy = dYear == null ? getYearNow() : dYear;
        String MM = dMonth == null ? getMonthNow() : dMonth;
     ...
     * @return
     */
    private static String[] dateCalculationFull(String dYear, String dMonth, String dDay,
